...
- [ ] Task 1: Create research.test.js file structure (AC: #1)
  - [ ] 1.1: Create `create-scrum-workflow/test/integration/research.test.js` with vitest imports and mock setup
  - [ ] 1.2: Define test constants for all 8 skills (6 existing + 2 research)
  - [ ] 1.3: Set up beforeEach/afterEach hooks for mock cleanup
  - [ ] 1.4: Create mock configurations for single-platform and multi-platform scenarios

- [ ] Task 2: Implement skill file existence tests (AC: #2)
  - [ ] 2.1: Test that both `scrum-research-technical` and `scrum-research-general` exist after installation
  - [ ] 2.2: Test skill directory structure: `{platform}/skills/{skill-name}/SKILL.md`
  - [ ] 2.3: Test all 8 skills exist (4 original + 2 docs + 2 research)
  - [ ] 2.4: Test multi-platform skill existence (claude-code, cursor, windsurf, etc.)

- [ ] Task 3: Implement framework path placeholder tests (AC: #3)
  - [ ] 3.1: Test `{{framework_path}}` is substituted with `scrum_workflow` in research-technical
  - [ ] 3.2: Test `{{framework_path}}` is substituted with `scrum_workflow` in research-general
  - [ ] 3.3: Test substituted content references correct command files (`commands/research-technical.md`, `commands/research-general.md`)
  - [ ] 3.4: Test edge case: framework path with special characters

- [ ] Task 4: Implement lock file tests (AC: #4)
  - [ ] 4.1: Test lock file contains entries for both research skill files
  - [ ] 4.2: Test SHA-256 hash format validation for research skill entries
  - [ ] 4.3: Test lock file structure includes all 8 skills

- [ ] Task 5: Implement research agent invocation tests (AC: #5)
  - [ ] 5.1: Test research-technical skill shim references correct command file
  - [ ] 5.2: Test research-general skill shim references correct command file
  - [ ] 5.3: Test skill shim frontmatter includes required fields (`name`, `description`, `active_in`)
  - [ ] 5.4: Test skill shim content includes workflow pattern descriptions

- [ ] Task 6: Implement WebSearch tool tests (AC: #6)
  - [ ] 6.1: Create mock for WebSearch tool invocation
  - [ ] 6.2: Test technical research workflow includes WebSearch calls
  - [ ] 6.3: Test general research workflow includes WebSearch calls
  - [ ] 6.4: Test search results are properly formatted and passed to synthesis

- [ ] Task 7: Implement output file creation tests (AC: #7)
  - [ ] 7.1: Test output files created in `docs/research/` directory
  - [ ] 7.2: Test technical research filename pattern: `technical-research-{topic-slug}-{date}.md`
  - [ ] 7.3: Test general research filename pattern: `general-research-{topic-slug}-{date}.md`
  - [ ] 7.4: Test `docs/research/` directory is created if it does not exist

- [ ] Task 8: Implement frontmatter schema validation tests (AC: #8)
  - [ ] 8.1: Test technical research frontmatter includes required fields: `type`, `topic`, `date`, `sources`, `ai_optimized`, `version`, `research_confidence`
  - [ ] 8.2: Test general research frontmatter includes required fields
  - [ ] 8.3: Test `type` field equals `technical_research` for technical mode
  - [ ] 8.4: Test `type` field equals `general_research` for general mode
  - [ ] 8.5: Test `ai_optimized` field is set to `true`

- [ ] Task 9: Implement state file management tests (AC: #9)
  - [ ] 9.1: Test `.research-state.json` is created in `docs/research/`
  - [ ] 9.2: Test state file includes `lastRun`, `topic`, `sources`, `phase` fields
  - [ ] 9.3: Test state file is updated after each research phase
  - [ ] 9.4: Test state file structure is valid JSON

- [ ] Task 10: Implement resume capability tests (AC: #10)
  - [ ] 10.1: Test interrupted research can resume from last completed phase
  - [ ] 10.2: Test resume reads state file correctly
  - [ ] 10.3: Test resume continues from correct phase (not from beginning)
  - [ ] 10.4: Test completed research does not trigger resume

- [ ] Task 11: Implement update mode tests (AC: #11)
  - [ ] 11.1: Test `--update` flag triggers incremental update mode
  - [ ] 11.2: Test update mode reads existing research document
  - [ ] 11.3: Test update mode compares new findings against existing content
  - [ ] 11.4: Test update mode presents diff summary before writing
  - [ ] 11.5: Test update mode preserves unchanged content

- [ ] Task 12: Validate test suite execution (AC: #12)
  - [ ] 12.1: Run `npm test` and verify all research tests pass
  - [ ] 12.2: Verify test coverage includes single-platform scenarios
  - [ ] 12.3: Verify test coverage includes multi-platform scenarios
  - [ ] 12.4: Verify tests complete in reasonable time (< 30 seconds)
