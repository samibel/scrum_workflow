...
├── context/                        ← Generated by /scrum-create-project-context
...
- Delta-update mechanism for project context (`/scrum-update-project-context`)
...
**Decision:** Add `/scrum-create-project-context` as the first command in the workflow — before any ticket work begins. This is a new Phase 0 not explicitly in the PRD but architecturally required to solve the context-mismatch problem identified in PRD Journey 4.

**Workflow Sequence (Updated):**
```
/scrum-create-project-context → /scrum-create-ticket → /scrum-refine-ticket → /scrum-dev-story → approve
```
...
- Adaptive: `/scrum-create-project-context` generates only files for detected domains — no empty files for missing domains
...
```
draft → refinement                     (trigger: /scrum-refine-ticket)
refinement → ready                     (guard: readiness check PASS)
refinement → draft                     (guard: readiness check FAIL)
ready → in-dev                         (guard: status == ready, trigger: /scrum-dev-story)
in-dev → in-review                     (trigger: /scrum-dev-story review)
...
|---|---|---|---|
| /scrum-create-ticket | /scrum-refine-ticket | `story.md` | YAML frontmatter + story description |
| /scrum-refine-ticket | Readiness Check | `story.md` (updated) + `refinement.md` | Updated story + agent perspectives |
| Readiness Check | /scrum-dev-story | `plan.md` | Execution plan with subtasks |
| /scrum-dev-story | Review | Code changes + `review-1.md` | Findings with severity and fixes |
...
2. `/scrum-create-project-context` command + workflow — depends on templates
3. Story file schema + state machine — foundation for all other commands
4. `/scrum-create-ticket` command + workflow — first ticket workflow command
5. Agent definitions (Architect, Dev, QA) in SKILL.md format — needed for refinement
6. `/scrum-refine-ticket` command + workflow — depends on agents
7. Readiness check (embedded in refinement workflow) — gates dev phase
8. `/scrum-dev-story` command + workflow — implementation + single review
...
- /scrum-refine-ticket depends on: agent definitions, /scrum-create-ticket output
- /scrum-dev-story depends on: readiness check, /scrum-refine-ticket output
...
| Command names | `scrum-` prefix + `kebab-case` with slash | `/scrum-create-ticket`, `/scrum-refine-ticket` |
...
All user-facing commands use the `scrum-` prefix for namespace isolation. This ensures scrum_workflow commands do not collide with other tools when installed in the same project.

| Internal File | Registered Skill Name | User Trigger |
|--------------|----------------------|-------------|
| `commands/create-project-context.md` | `scrum-create-project-context` | `/scrum-create-project-context` |
| `commands/create-ticket.md` | `scrum-create-ticket` | `/scrum-create-ticket` |
| `commands/refine-ticket.md` | `scrum-refine-ticket` | `/scrum-refine-ticket` |
| `commands/dev-story.md` | `scrum-dev-story` | `/scrum-dev-story` |

Internal framework file names (under `scrum_workflow/commands/`) do NOT use the prefix — only the registered skill names and user-facing triggers do.
...
| Situation | Behavior |
|---|---|
| Command on wrong status | Error: "Story SW-101 is in status `draft`, but `/scrum-dev-story` requires `ready`" |
| Required file missing | Error: "File `sprints/SW-101/story.md` not found. Run `/scrum-create-ticket SW-101` first" |
...
|---|---|---|
| /scrum-create-ticket | `story.md` | `refinement.md`, `plan.md`, `review-*.md` |
| /scrum-refine-ticket | `refinement.md`, `story.md` (update) | `plan.md`, `review-*.md` |
| Readiness check | `plan.md`, `story.md` (status update) | `refinement.md`, `review-*.md` |
| /scrum-dev-story | Code files, `review-1.md` | `story.md`, `refinement.md`, `plan.md` |
...
```
project-root/
├── context/                                 ← Generated by /scrum-create-project-context (FACTS)
...
│   └── architecture.md                      ← (if detected)
│
├── skills/                                  ← Generated by /scrum-create-project-context (HOW TO WORK)
...
│   ├── SW-101/
│   │   ├── story.md                         ← Created by /scrum-create-ticket
│   │   ├── refinement.md                    ← Created by /scrum-refine-ticket
│   │   ├── plan.md                          ← Created by readiness check
│   │   ├── review-1.md                      ← Created by /scrum-dev-story review
...
| **Domain Skills** | `project-root/skills/` (project) | `/scrum-create-project-context` | HOW to work in THIS project (specific, generated from codebase analysis) |
...
| State → Framework | Sprint files NEVER modify framework files | `/scrum-dev-story` editing `scrum_workflow/templates/story.md` |

**Phase Boundaries (Write Rules):**

| Phase | Reads | Writes |
|---|---|---|
| /scrum-create-project-context | Project codebase (scan) | `context/*.md`, `skills/*/SKILL.md` |
| /scrum-create-ticket | `context/index.md`, relevant `context/*.md` | `sprints/SW-XXX/story.md` |
| /scrum-refine-ticket | `story.md`, relevant `context/*.md`, relevant `skills/` | `refinement.md`, `story.md` (update) |
| Readiness check | `story.md`, `refinement.md` | `plan.md`, `story.md` (status) |
| /scrum-dev-story | `story.md`, `plan.md`, relevant `context/*.md`, relevant `skills/` | Code files, `review-1.md` |
...
- Delta-update for project context (`/scrum-update-project-context`)
- `index.yaml` sprint index for `/scrum-status` command
...
2. `/scrum-create-project-context` command + workflow
3. Story file schema + state machine validation
4. `/scrum-create-ticket` command + workflow
5. Agent definitions (Architect, Dev, QA) in SKILL.md format
6. `/scrum-refine-ticket` command + workflow
