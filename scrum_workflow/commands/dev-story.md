...
- `ready` → `in-dev` (via `/scrum-dev-story` - THIS COMMAND)
- `in-dev` → `in-review` (via `/scrum-dev-story review`)
- `in-review` → `done` (via human approval)

## Error Handling

- If story file does not exist, provide actionable error suggesting `/scrum-create-ticket`
- If status is not `ready`, provide actionable error suggesting `/scrum-refine-ticket`
