...
|---|---|---|
| `/create-ticket` | No status (new story) | Story file must not exist |
| `/refine-ticket` | `draft` | Story must be in `draft` status |
| Readiness check | `refinement` | Story must be in `refinement` status |
| `/dev-story` | `ready` | Story must be in `ready` status |
...
### Create Ticket Command (`/create-ticket`)
...
### Refine Ticket Command (`/refine-ticket`)
...
```
Error: Story SW-XXX is in status 'current_status', but '/refine-ticket' requires 'draft'
...
Fix: Run '/refine-ticket SW-XXX' first to complete refinement
```

### Dev Story Command (`/dev-story`)
...
Fix: Run '/refine-ticket SW-XXX' and pass readiness check before starting development
```
...
- Skipping phases (e.g., running `/dev-story` on a `draft` story)
