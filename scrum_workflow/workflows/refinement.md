...
- Verify status is `draft` (required for `/refine-ticket`)
...
**Expected input format:** `/refine-ticket SW-XXX`
...
```
Error: Project context not found. Run '/create-project-context' first
Fix: Run '/create-project-context' to generate project context files before refinement
```
