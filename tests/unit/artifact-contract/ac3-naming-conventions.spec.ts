...
describe('AC3: Story ID Format — SW-XXX naming convention', () => {
  ...
  test('[P0] create-ticket command should specify SW-XXX (3-digit, zero-padded) story ID format', () => {
  ...
  test('[P0] ticket-creation workflow should use SW-XXX format for story directory naming', () => {
  ...
  test('[P1] Story should document the story ID format delta (SW-XXX spec vs X-Y-name.md implementation)', () => {
...
describe('AC3: Review Artifact Naming — review-{N}.md convention', () => {
  ...
  test('[P0] review-story command should specify review-{N}.md sequential naming', () => {
  ...
  test('[P0] review-story workflow should specify sequential numbering for review artifacts', () => {
...
describe('AC3: Approval Artifact Naming — approval-{N}.md convention (deferred)', () => {
  ...
  test('[P0] Story should document approval-{N}.md naming convention as deferred to Epic 2', () => {
    const content = readFileSync(storyFile, 'utf8');
...
describe('AC3: Research Report Naming — RR-XXX.md convention', () => {
  ...
  test('[P0] research-technical command should specify RR-XXX.md (3-digit zero-padded) naming', () => {
  ...
  test('[P0] research-general command should specify RR-XXX.md naming convention', () => {
  ...
  test('[P0] research-technical workflow should generate RR-XXX.md filenames', () => {
  ...
  test('[P0] Existing research artifacts in _scrum-output/memory/research/ should follow RR-XXX.md convention', () => {
...
describe('AC3: Decision Record Naming — DR-XXX.md convention (deferred, Epic 7)', () => {
  ...
  test('[P1] Story should reference DR-XXX.md naming convention (deferred)', () => {
    const content = readFileSync(storyFile, 'utf8');
...
describe('AC3: Risk Note Naming — RN-XXX.md convention (deferred, Epic 7)', () => {
  ...
  test('[P1] Story should reference RN-XXX.md naming convention (deferred)', () => {
    const content = readFileSync(storyFile, 'utf8');
...
describe('AC3: Session Summary Naming — session-{YYYY-MM-DD}.md convention (deferred, Epic 7)', () => {
  ...
  test('[P1] Story should reference session-{YYYY-MM-DD}.md naming convention (deferred)', () => {
    const content = readFileSync(storyFile, 'utf8');
...
describe('AC3: Architecture Specification — Naming Patterns Reference', () => {
  ...
  test('[P0] Architecture should define SW-XXX (3-digit, zero-padded) story ID format', () => {
    const content = readFileSync(architectureFile, 'utf8');
    expect(content).toMatch(/SW-XXX|SW-\d{3}|zero-padded/i);
  });

  // Test 3.16: Architecture defines review-{N}.md naming
  test('[P0] Architecture should define review-{N}.md sequential naming', () => {
    const content = readFileSync(architectureFile, 'utf8');
    expect(content).toMatch(/review-\{N\}|review-N\.md|review-1\.md/i);
  });

  // Test 3.17: Architecture defines RR-XXX.md naming
  test('[P0] Architecture should define RR-XXX.md (3-digit zero-padded) naming', () => {
    const content = readFileSync(architectureFile, 'utf8');
    expect(content).toMatch(/RR-XXX\.md|RR-\d{3}\.md/);
  });

  // Test 3.18: Architecture defines code naming conventions (SKILL.md, workflow.md)
  test('[P1] Architecture should define code naming conventions (SKILL.md uppercase, workflow.md lowercase)', () => {
    const content = readFileSync(architectureFile, 'utf8');
