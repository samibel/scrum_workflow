...
describe('AC1: Story file contains delta analysis', () => {
  ...
  test.skip('[P0] Story 3.1 implementation file should exist', () => {
    expect(existsSync(STORY_FILE)).toBe(true);
  });

  // Test 1.2: Story documents delta analysis between current implementation and PRD
  test.skip('[P0] Story should document delta analysis between implementation and PRD FR-4', () => {
  ...
  test.skip('[P0] Story should document all 9 FR-4 states explicitly', () => {
    const content = readFileSync(STORY_FILE, 'utf8');
  ...
  test.skip('[P0] Story should document the refinement state discrepancy vs FR-4', () => {
    const content = readFileSync(STORY_FILE, 'utf8');
  ...
  test.skip('[P0] Story should document the missing any→cancelled transition gap', () => {
    const content = readFileSync(STORY_FILE, 'utf8');
  ...
  test.skip('[P0] Story should document resolutions for all identified deltas', () => {
    const content = readFileSync(STORY_FILE, 'utf8');
  ...
  test.skip('[P0] All story tasks should be completed (no unchecked boxes)', () => {
    const content = readFileSync(STORY_FILE, 'utf8');
...
describe('AC1: standards.md — Story Status State Machine section contains all deltas resolved', () => {
  ...
  test.skip('[P0] standards.md should exist', () => {
    expect(existsSync(STANDARDS_FILE)).toBe(true);
  });

  // Test 1.9: standards.md contains a Story Status State Machine section
  test.skip('[P0] standards.md should contain Story Status State Machine section', () => {
  ...
  test.skip('[P0] standards.md State Machine section should list all 9 FR-4 states', () => {
    const content = readFileSync(STANDARDS_FILE, 'utf8');
    expect(content).toMatch(/\bdraft\b/);
    expect(content).toMatch(/\brefined\b/);
    expect(content).toMatch(/\bready-for-dev\b/);
  ...
  test.skip('[P1] standards.md should document refinement state with FR-4 deviation note', () => {
    const content = readFileSync(STANDARDS_FILE, 'utf8');
    expect(content).toMatch(/\brefinement\b/);
  ...
  test.skip('[P0] standards.md Valid Transitions table should include any→cancelled transition', () => {
    const content = readFileSync(STANDARDS_FILE, 'utf8');
...
describe('AC1: status-guard-validation/SKILL.md — contains resolved delta', () => {
  ...
  test.skip('[P0] SKILL.md valid status values should include cancelled', () => {
    const content = readFileSync(STATUS_GUARD_SKILL, 'utf8');
    expect(content).toMatch(/\bcancelled\b/);
  });

  // Test 1.18: SKILL.md valid transitions includes any→cancelled
  test.skip('[P0] SKILL.md valid transitions should include any→cancelled', () => {
    const content = readFileSync(STATUS_GUARD_SKILL, 'utf8');
    expect(content).toMatch(/any.*cancelled|any →.*cancelled|`any`.*`cancelled`/i);
  });
});
