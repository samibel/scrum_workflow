...
describe('AC3: SKILL.md — No silent failure policy', () => {
  ...
  test.skip('[P0] SKILL.md should include **Details:** in every guard failure (no silent failures)', () => {
  ...
  test.skip('[P0] SKILL.md should include **Next Step:** in every guard failure (actionable messages)', () => {
    const content = readFileSync(SKILL_FILE, 'utf8');
  ...
  test.skip('[P0] SKILL.md should state that guard checks occur BEFORE any file writes', () => {
    const content = readFileSync(SKILL_FILE, 'utf8');
  ...
  test.skip('[P0] SKILL.md should state that no command leaves story in inconsistent state', () => {
    const content = readFileSync(SKILL_FILE, 'utf8');
  ...
  test.skip('[P0] SKILL.md guard errors should reference the command that was attempted', () => {
    const content = readFileSync(SKILL_FILE, 'utf8');
  ...
  test.skip('[P0] SKILL.md guard errors should identify what failed: current status vs required status', () => {
    const content = readFileSync(SKILL_FILE, 'utf8');
  ...
  test.skip('[P0] SKILL.md should declare the skill never writes files (read-only validation)', () => {
    const content = readFileSync(SKILL_FILE, 'utf8');
...
describe('AC3: Command files — actionable error messages with Next Step', () => {
  ...
  test.skip('[P0] commands/dev-story.md guard error should include actionable next step', () => {
  ...
  test.skip('[P0] commands/approve.md error handling should include actionable next step', () => {
    const content = readFileSync(APPROVE_CMD, 'utf8');
    expect(content).toMatch(/\*\*Next Step:\*\*|next step|run.*\/scrum-/i);
  });

  // Test 3.11: review-story.md includes actionable next step
  test.skip('[P0] commands/review-story.md guard error should include actionable next step', () => {
    const content = readFileSync(REVIEW_STORY_CMD, 'utf8');
    expect(content).toMatch(/\*\*Next Step:\*\*|next step|run.*\/scrum-/i);
  });

  // Test 3.12: refine-ticket.md includes actionable next step
  test.skip('[P0] commands/refine-ticket.md guard error should include actionable next step', () => {
    const content = readFileSync(REFINE_TICKET_CMD, 'utf8');
    expect(content).toMatch(/\*\*Next Step:\*\*|next step|run.*\/scrum-/i);
  });

  // Test 3.13: refine-story.md includes actionable next step
  test.skip('[P0] commands/refine-story.md guard error should include actionable next step', () => {
    const content = readFileSync(REFINE_STORY_CMD, 'utf8');
    expect(content).toMatch(/\*\*Next Step:\*\*|next step|run.*\/scrum-/i);
  });

  // Test 3.14: create-ticket.md includes actionable next step
  test.skip('[P0] commands/create-ticket.md guard error should include actionable next step', () => {
...
describe('AC3: SKILL.md and command files — old silent error format eliminated', () => {
  ...
  test.skip('[P1] commands/dev-story.md should NOT use plain "Error:" prefix for guard violations', () => {
    const content = readFileSync(DEV_STORY_CMD, 'utf8');
